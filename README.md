...
**Decision-maker:** http://127.0.0.1:8080/ (**AI 办公室** — 目标列表 + 新建目标 + 填充槽含「我来填」+ 只读 **工位心跳**). Inbox / 待我拍板 is a top-bar **drawer** (`待办 · n`); http://127.0.0.1:8080/inbox stays loadable. Roster paints `presence` / `last_seen_at` (`last_heartbeat`) / `ttl_seconds` / `heartbeat_fresh` (在忙 / 等证据 / 空闲；TTL 90s) — stale heartbeat is never 在忙. 「我来填」is note + optional artifact, not assign/dispatch. Bot completion writes (dispatch / evidence) require `X-Harness-Entry: mcp`. Health / OpenAPI / outbox stay on http://127.0.0.1:8080/ops (403 for `decision_maker`). AUTHORITATIVE IA: [OFFICE_HOME_IA_P0_v1.md](services/boundary-harness/docs/experience/OFFICE_HOME_IA_P0_v1.md). Smoke: [FRONTEND_OFFICE_P0_SMOKE.md](services/boundary-harness/docs/FRONTEND_OFFICE_P0_SMOKE.md). Dogfood SOP: [DOGFOOD_GROKBOT_MCP_SOP.md](services/boundary-harness/docs/DOGFOOD_GROKBOT_MCP_SOP.md).
