...
| Path | Role |
|------|------|
| `services/boundary-harness/apps/api` | Domain HTTP + decision-maker Inbox `/inbox` + ops `/ops` |
...
|------|-------------|--------------|
| **Decision-maker (default)** | `/` or `/inbox` | AI 办公室 + 待我拍板. Inbox top bar: only `待办` / `待办 · n`. Weak `工位一览`. No Health / OpenAPI / outbox / role switch. |
