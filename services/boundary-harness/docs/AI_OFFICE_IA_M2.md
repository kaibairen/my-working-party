...
P1 **工位心跳** is a read-only presence projection (`GET /v1/desks`): avatar / name / status dot (`在忙` | `等证据` | `空闲`). Live bots POST `/v1/agents/heartbeat` (TTL 90s default); expired heartbeats fall back to `pool_seed`. It MUST NOT become dispatch or drag-to-assign. Empty fill slots may show **我来填** (`human_allowed` or `exception_grant`) — still a projection, not a dispatch board. Sole hard HITL remains 待我拍板 / Gate decide (drawer).
