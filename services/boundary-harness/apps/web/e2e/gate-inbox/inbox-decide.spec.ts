...
test.describe("E2E decide", () => {
  test("decide_pass_uses_version", async ({ page, baseURL }) => {
    ...
    await expect(page.getByTestId("inbox-flash")).toContainText(/succeeded|已通过/i);
  ...
  test("decide_revise_default_same_assignment", async ({ page, baseURL }) => {
    ...
    await expect(page.getByTestId("inbox-flash")).toContainText(/succeeded|已打回重做/i);
  ...
  test("decide_optimistic_lock_409_refresh", async ({ page, baseURL }) => {
    ...
    await page.getByTestId("decide-pass").click();
    await expect(page.getByTestId("inbox-flash")).toContainText(/有人刚处理过，已刷新|conflict|optimistic_lock/i);
