...
P1 **工位心跳** is a read-only presence projection (`GET /v1/desks`): avatar / name / status dot (`在忙` | `等证据` | `空闲`), grouped on the office right rail. Default roster is heartbeat agents only (TTL 90s); seed pools are hidden from the decision-maker list. It MUST NOT become dispatch or drag-to-assign. Sole hard HITL remains 待我拍板 / Gate decide (drawer).
