...
P1 **工位心跳** is a read-only presence projection (`GET /v1/desks`): avatar / name / status dot (`在忙` | `等证据` | `空闲`). Default list is live heartbeat only (TTL 90s); seed placeholders are omitted. It MUST NOT become dispatch or drag-to-assign. Sole hard HITL remains 待我拍板 / Gate decide (drawer).
