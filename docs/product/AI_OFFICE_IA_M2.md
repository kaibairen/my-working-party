...
P1 **工位心跳** is a read-only presence projection (`GET /v1/desks`): avatar / name / status dot (`在忙` | `等证据` | `空闲`). Live bots POST `/v1/agents/heartbeat` (TTL 90s). It MUST NOT become dispatch or drag-to-assign. Sole hard HITL remains 待我拍板 / Gate decide (drawer).
